...
pub async fn add_sys_dict_data(item: Json<AddDictDataReq>) -> AppResult<Response> {
...
pub async fn update_sys_dict_data(item: Json<UpdateDictDataReq>) -> AppResult<Response> {
    ...
    DictData::update_by_map(rb, &sys_dict_data, value! {"id": &req.id}).await?;
    ok_result()
}
