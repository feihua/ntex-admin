...
pub async fn add_sys_menu(item: Json<AddMenuReq>) -> AppResult<Response> {
...
pub async fn update_sys_menu(item: Json<UpdateMenuReq>) -> AppResult<Response> {
    ...
    Menu::update_by_map(rb, &sys_menu, value! {"id": &req.id}).await?;
